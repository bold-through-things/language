...
def expand_union_types(arg_list, type_definitions):
    """
    Expand union types in arguments to create multiple overload combinations.
    Returns a list of argument lists, one for each combination of union types.
    """
    if not arg_list:
        return [[]]
    
    # Get all possible types for the first argument
    first_arg = arg_list[0]
    first_arg_types = get_all_types_for_argument(first_arg, type_definitions)
    
    # Recursively expand the rest of the arguments
    rest_combinations = expand_union_types(arg_list[1:], type_definitions)
    
    # Combine first argument types with rest combinations
    result = []
    for first_type in first_arg_types:
        for rest_combo in rest_combinations:
            result.append([first_type] + rest_combo)
    
    return result

def get_all_types_for_argument(arg, type_definitions):
    """
    Get all possible 67lang types for an argument, expanding unions.
    """
    idl_type = arg["idlType"]
    
    # Check if this is a union type
    if idl_type.get("union", False):
        # This is a union type, return all possible types
        union_types = []
        for variant in idl_type["idlType"]:
            union_types.append(idl_type_to_67lang_type(variant, type_definitions))
        return union_types
    else:
        # This is a single type
        return [idl_type_to_67lang_type(idl_type, type_definitions)]
...
def main():
                            ...
                            for member in definition["members"]:
                                try:
                                    if member["type"] == "constructor":
                                        ...
                                        for i in range(min_args, len(arguments) + 1):
                                            # Expand union types to create multiple overloads
                                            arg_combinations = expand_union_types(arguments[:i], type_definitions)
                                            returns = idl_type_to_67lang_type(member["idlType"], type_definitions)
                                            
                                            for demands in arg_combinations:
                                                if name == "Window": # All non-static methods of Window are treated as global DirectCalls
                                                    ...
                                                    if member["name"] == "setInterval":
                                                        print(f"INTERFACE DEBUG: Generated PrototypeCall for setInterval: constructor={name}")
                                        ...
                                        for member in mixin_operations[mixin_name]:
                                            if member["type"] == "operation":
                                                ...
                                                for i in range(min_args, len(arguments) + 1):
                                                    ...
                                                    for demands in arg_combinations:
                                                        # For mixin operations, generate DirectCall (global functions) instead of PrototypeCall
                                                        # since mixins like WindowOrWorkerGlobalScope provide global functions
                                                        if name == "Window" or mixin_name == "WindowOrWorkerGlobalScope":
                                                            call = DirectCall(fn=member["name"], receiver=None, demands=demands, returns=returns)
                                                        else:
                                                            instance_demands = [name] + demands # 'name' is the including interface
                                                            call = PrototypeCall(constructor=name, fn=member["name"], demands=instance_demands, returns=returns)
                                                            if member["name"] == "setInterval":
                                                                print(f"MIXIN DEBUG: Generated PrototypeCall for setInterval: constructor={name}, mixin={mixin_name}")
                ...
                for member in mixin_operations[mixin_name]:
                    if member["type"] == "operation":
                        ...
                        for i in range(min_args, len(arguments) + 1):
                            ...
                            # WindowOrWorkerGlobalScope methods should be global functions, not instance methods
                            if mixin_name == "WindowOrWorkerGlobalScope":
                                call = DirectCall(fn=member["name"], receiver=None, demands=demands, returns=returns)
                            elif interface_name == "Window":
                                call = DirectCall(fn=member["name"], receiver=None, demands=demands, returns=returns)
                            else:
                                # Other mixin operations become prototype calls on the including interface
    ...
    # Deduplicate calls before writing
    for name, calls in generated_builtins.items():
        seen = []
        for call in calls:
            if call not in seen:
                seen.append(call)
        generated_builtins[name] = seen
