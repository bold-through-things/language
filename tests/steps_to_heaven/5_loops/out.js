...
void (async () => {
    ...
    const scope = globalThis;
    {
        const parent_scope = scope
        {
            ...
            let _0x0_numbers = [1, 2, 3]
            _0x0_numbers
            let _0x2__0x1_numbers = _0x0_numbers

            const _0x8_iter = _0x2__0x1_numbers[Symbol.iterator]();
            while (true) {
                const { value, done } = _0x8_iter.next();
                if (done) { break; }
                let num = value;
                {
                    const parent_scope = scope
                    {
                        const scope = _67lang.scope(parent_scope)
                        let _0x3_num = num
                        const _0x9_await__67lang_dot_log_lp_ = await _67lang.log("Number:", _0x3_num)
                        _0x9_await__67lang_dot_log_lp_
                    }
                } }
            let _0x4_names = ["Alice", "Bob"]
            _0x4_names
            let _0x6__0x5_names = _0x4_names

            const _0xa_iter = _0x6__0x5_names[Symbol.iterator]();
            while (true) {
                const { value, done } = _0xa_iter.next();
                if (done) { break; }
                let name = value;
                {
                    const parent_scope = scope
                    {
                        const scope = _67lang.scope(parent_scope)
                        let _0x7_name = name
                        const _0xb_await__67lang_dot_log_lp_ = await _67lang.log("Hello", _0x7_name)
